...
import SavedPurchaseOrders from "./SavedPurchaseOrders";
...
const PurchaseOrder = () => {
  ...
  const deliveryPeriod =
    specs.find((s) => s.name.toLowerCase() === "delivery")?.value || "2 weeks";
  const totalAmount =
    specs.find((s) => s.name.toLowerCase() === "total amount")?.value || 0;
